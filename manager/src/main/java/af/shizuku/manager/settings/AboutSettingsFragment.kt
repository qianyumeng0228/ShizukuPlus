...
class AboutSettingsFragment : BaseSettingsFragment() {
    ...
    override fun onCreateSettingsPreferences(savedInstanceState: Bundle?, rootKey: String?) {
        ...
        findPreference<Preference>("source_code")?.setOnPreferenceClickListener {
            CustomTabsHelper.launchUrlOrCopy(context, ReleaseConfig.GITHUB_URL)
            true
        }

        findPreference<Preference>("open_source_licenses")?.setOnPreferenceClickListener {
            CustomTabsHelper.launchUrlOrCopy(requireContext(), "${ReleaseConfig.GITHUB_URL}/blob/master/OPEN_SOURCE_LICENSES.md")
            true
        }
    ...
    private fun openReleasesPage() {
        try {
            startActivity(
                Intent(Intent.ACTION_VIEW, Uri.parse(ReleaseConfig.RELEASES_URL))
